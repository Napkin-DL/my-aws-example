...
class DilConvDecoder(nn.Module):
    ...
    def __init__(self, latent_input_size, args, token_weights=None,
        ...
        # self.nvidia_measure("init function")

    def nvidia_measure(self, name):
        import GPUtil as GPU
        GPUs = GPU.getGPUs()
        gpu = GPUs[self.host_rank]
        print("HOST_NUM : {0}, Name : {1}, GPU RAM Free: {2:.0f}MB | Used: {3:.0f}MB | Util {4:3.0f}% | Total {5:.0f}MB".format(
            self.host_rank, name, gpu.memoryFree, gpu.memoryUsed, gpu.memoryUtil*100, gpu.memoryTotal))

        print('Device Name : {}, Allocated: {} GB, Cached: {} GB'.format(torch.cuda.get_device_name(self.host_rank), round(
            torch.cuda.memory_allocated(self.host_rank)/1024**3, 1), round(torch.cuda.memory_cached(self.host_rank)/1024**3, 1)))
    ...
    def get_logits(self, input_tensor, z, sampling=False):
        ...
        '''
        # self.nvidia_measure("get_logits_start")
        ...
        # self.nvidia_measure("get_logits_end")
    ...
    def get_log_prob(self, x, z):
        ...
        '''
        # self.nvidia_measure("get_log_prob_start")
        seq_logits = torch.gather(self.get_logits(x, z)[:, :-1, :],
                                  2, x[:, 1:].long().unsqueeze(-1))
        # self.nvidia_measure("get_log_prob_end")
        return seq_logits[:, :, 0]
    ...
    def weighted_forward(self, sm_list, z):
        '''
        '''
        # self.nvidia_measure("weighted_forward_start")
        ...
        # self.nvidia_measure("weighted_forward_end")
        return ans_logits
